...
public class LogicSender : ILogicSenderCong
{
    ...
    public LogicSender(HttpClient httpClient, ILogger<LogicSender> logger)
    {
    ...
    public async Task<string?> GetCongrStrAsync(int wishId)
    {
        ...
        var response = await _httpClient.GetAsync($"/api/mailbot/GetCongrString/{wishId}");
    ...
    public async Task<string> GetEmailAsync(Guid AppId)
    {
        ...
        var response = await _httpClient.GetAsync($"/api/mailbot/email/{AppId}");
        if (!response.IsSuccessStatusCode)
        {
    ...
    public async Task<List<FriendDto>> GetTodayBirthdayAsync()
    {
        ...
        var response = await _httpClient.GetAsync($"/api/mailbot/birthdays/today");
        if (!response.IsSuccessStatusCode)
        {
    ...
    public async Task<PozdrikIdDto?> GetWishIdAsync(FriendDto friendDto)
    {
        ...
        var response = await _httpClient.PostAsJsonAsync($"/api/mailbot/getPozdrikId", friendDto);
