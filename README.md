...
- Использует Kafka как брокер сообщений для обработки асинхронных задач (если подключено).

### Внешние сервисы:
- SMTP-сервер для отправки писем.
- Kafka (если настроено).
...
| KAFKA_BROKER    | Адрес брокера Kafka (если используется) |
